...
#define MIICAM_DYNAMIC_DEFECT_T1_MIN    10      /* dynamic defect pixel correction, threshold, means: 1.0 */
#define MIICAM_DYNAMIC_DEFECT_T1_MAX    100     /* means: 10.0 */
#define MIICAM_DYNAMIC_DEFECT_T1_DEF    13      /* means: 1.3 */
#define MIICAM_DYNAMIC_DEFECT_T2_MIN    0       /* dynamic defect pixel correction, value, means: 0.00 */
#define MIICAM_DYNAMIC_DEFECT_T2_MAX    100     /* means: 1.00 */
#define MIICAM_DYNAMIC_DEFECT_T2_DEF    100
...
/*
    nWaitMS: The timeout interval, in milliseconds. If a nonzero value is specified, the function waits until the image is ok or the interval elapses.
             If nWaitMS is zero, the function does not enter a wait state if the image is not available; it always returns immediately; this is equal to Miicam_PullImageV3.
...
/*  for pull mode: MIICAM_EVENT_STILLIMAGE, and then Miicam_PullStillImageXXXX/Miicam_PullImageV3
...
* piValue: output, MIICAM_PIXELFORMAT_xxxx
*/
MIICAM_API(HRESULT)     Miicam_get_PixelFormatSupport(HMiicam h, char cmd, int* piValue);
MIICAM_API(const char*) Miicam_get_PixelFormatName(int val);
